...
Livewords.utilities = {
	...
	findLetterDist : function(words) {
		...
		for (var letter in this.letterDist) {
			var fullBagDist = Math.ceil((this.letterDist[letter][2] / this.numLetters) * 100);
			var halfBagDist = Math.ceil(fullBagDist / 2);
			var thirdBagDist = Math.ceil(fullBagDist / 3);

			numfullBag += fullBagDist;
			numhalfBag += halfBagDist;
			numthirdBag += thirdBagDist;
		...
		// console.log("full bag: " + numfullBag);
		// console.log("half bag: " + numhalfBag);
		// console.log("third bag: " + numthirdBag);
	},
